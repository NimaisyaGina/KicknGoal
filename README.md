...
--> Model Dart untuk JSON dibutuhkan agar data yang diterima atau dikirim memiliki tipe yang jelas dan aman. Jika langsung memetakan Map<String, dynamic> tanpa model, risiko error meningkat karena kita kehilangan validasi tipe, null-safety, dan struktur data yang konsisten. Selain itu, kode jadi sulit dipelihara, apalagi kalau struktur JSON berubah, karena semua pemanggilan data harus diubah manual.

2. Apa fungsi package http dan CookieRequest dalam tugas ini? Jelaskan perbedaan peran http vs CookieRequest.
--> Package http berfungsi untuk melakukan request HTTP sederhana seperti GET atau POST tanpa otomatis menyimpan cookie. Sedangkan CookieRequest menambahkan manajemen session dan cookie, sehingga Django bisa mengenali pengguna yang sudah login. Jadi http lebih stateless, sementara CookieRequest stateful untuk autentikasi.

3. Jelaskan mengapa instance CookieRequest perlu untuk dibagikan ke semua komponen di aplikasi Flutter.
--> Instance CookieRequest perlu dibagikan ke semua komponen Flutter agar status login, session, atau cookie bisa diakses di berbagai halaman. Kalau tiap halaman bikin instance baru, pengguna harus login ulang di setiap halaman dan data yang tergantung autentikasi tidak konsisten.

4. Jelaskan konfigurasi konektivitas yang diperlukan agar Flutter dapat berkomunikasi dengan Django. Mengapa kita perlu menambahkan 10.0.2.2 pada ALLOWED_HOSTS, mengaktifkan CORS dan pengaturan SameSite/cookie, dan menambahkan izin akses internet di Android? Apa yang akan terjadi jika konfigurasi tersebut tidak dilakukan dengan benar?
--> Agar Flutter bisa komunikasi dengan Django, konfigurasi penting adalah: menambahkan 10.0.2.2 di ALLOWED_HOSTS supaya emulator bisa akses server lokal, mengaktifkan CORS agar request dari domain berbeda diterima, pengaturan SameSite/cookie untuk menyimpan session di browser atau mobile, dan izin internet di AndroidManifest agar app bisa konek ke server. Kalau salah satu tidak dilakukan, request bisa gagal, login tidak tersimpan, atau data tidak tampil di app.

5. Jelaskan mekanisme pengiriman data mulai dari input hingga dapat ditampilkan pada Flutter.
--> Mekanisme pengiriman data mulai dari input di Flutter, dikemas menjadi JSON, dikirim via POST atau GET ke Django. Django menerima request, memprosesnya (misal menyimpan ke database), lalu mengembalikan response JSON. Flutter menerima JSON tersebut, di-deserialize ke model Dart, lalu data bisa ditampilkan di UI.

6. Jelaskan mekanisme autentikasi dari login, register, hingga logout. Mulai dari input data akun pada Flutter ke Django hingga selesainya proses autentikasi oleh Django dan tampilnya menu pada Flutter.
--> Untuk autentikasi, pengguna memasukkan username/password di Flutter, dikirim ke Django melalui request POST. Django memvalidasi data, jika berhasil akan buat session dan mengirim cookie balik. Di Flutter, cookie disimpan oleh CookieRequest, sehingga setiap halaman bisa mengenali user yang login. Logout dilakukan dengan request ke endpoint logout Django, session dihapus, dan menu Flutter diperbarui sesuai status login.

7. Jelaskan bagaimana cara kamu mengimplementasikan checklist di atas secara step-by-step! (bukan hanya sekadar mengikuti tutorial).
--> Pertama-tama, saya memastikan bahwa proyek Django saya sudah berjalan dengan stabil dan semua endpoint yang diperlukan sudah berfungsi, karena kestabilan backend sangat menentukan keberhasilan integrasi dengan Flutter. Setelah memastikan backend bekerja, saya mulai membangun koneksi antara Django dan Flutter dengan hati-hati, memastikan setiap request dan response JSON dapat diterima dan diproses dengan benar di aplikasi Flutter. Saya membuat model kustom di Django sesuai kebutuhan aplikasi, lalu di Flutter membuat model Dart yang sesuai agar data bisa di-mapping dengan tipe yang tepat dan aman. Selanjutnya, saya mengimplementasikan fitur registrasi dan login di Flutter dengan autentikasi yang terhubung ke Django, termasuk pengelolaan session dan cookie. Setelah itu, saya membangun halaman daftar produk yang mengambil data dari endpoint JSON Django, menampilkan atribut lengkap setiap item, dan menambahkan fitur filter agar hanya produk milik pengguna yang login yang terlihat. Saya juga menambahkan halaman detail untuk masing-masing produk dan tombol navigasi agar pengguna bisa kembali ke halaman daftar. Seluruh proses ini dilakukan secara bertahap, menguji setiap langkah untuk memastikan data mengalir dengan benar dari Django ke Flutter dan UI menampilkan informasi yang valid dan responsif.